const marketplaceListings = [
    {
        ...
        isNew: true,
        postedAt: '2 days ago',
        description: 'Excellent condition MacBook Pro 2021 with M1 Pro chip. 16GB RAM, 512GB SSD. Battery health at 92%. Includes original charger and box.',
        status: 'active',
...
export function getMarketplaceListing(id) {
    return marketplaceListings.find((listing) => listing.id === id);
}
function recomputeListingRating(listing) {
...
export function addMarketplaceListing(input) {
    const newListing = {
        ...input,
        id: Date.now(),
        views: 0,
...
export function addMarketplaceReview(listingId, review) {
...
export function likeMarketplaceListing(id) {
...
export function rsvpToEvent(id, change = 1) {
...
export function addForumThread(input) {
    const newThread = {
        ...input,
        id: Date.now(),
        replies: 0,
