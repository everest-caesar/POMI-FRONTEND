    ...
    return (
      <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-slate-400">You need to be signed in to create a listing.</p>
          <Link to="/marketplace">
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm text-slate-500">Redirecting to marketplace...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-white mb-2">Create a Listing</h1>
          <p className="text-slate-400">
            Share something with the community. Great photos and clear details help your listing shine.
          </p>
        </div>
        ...
        {!isAdmin && (
          <div className="mb-6 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4">
            <p className="text-sm text-amber-300">
              <strong>Note:</strong> All listings are reviewed by our admin team before going live.
              This helps ensure quality and safety for the community.
            </p>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="md:col-span-2">
                ...
                <textarea
                  ...
                  rows={5}
                  className="mt-2 w-full rounded-lg bg-slate-800 border border-slate-700 p-3 text-white placeholder:text-slate-500 focus:border-orange-500 focus:outline-none resize-none"
                  required
                />
              </div>
            </div>
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
            <Label className="text-white mb-4 block">Photos (Optional)</Label>
            <button
              type="button"
              onClick={() => fileInputRef.current?.click()}
              className="w-full rounded-lg border-2 border-dashed border-slate-700 p-8 text-center cursor-pointer hover:border-orange-500 transition-colors bg-slate-900/40"
            >
              <Upload className="h-8 w-8 text-slate-400 mx-auto mb-2" />
              <p className="text-sm text-slate-300">Click to upload or drag and drop</p>
              <p className="text-xs text-slate-500">JPEG, PNG, WebP up to 10MB (max 10 images)</p>
            </button>
            <input
              ref={fileInputRef}
              type="file"
            ...
            {uploadedImageUrls.length > 0 && (
              <p className="mt-3 text-xs font-semibold text-emerald-400">
                {uploadedImageUrls.length} image{uploadedImageUrls.length > 1 && "s"} ready to publish
              </p>
            )}
          </div>
